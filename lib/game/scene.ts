...
export class AbyssScene extends Phaser.Scene {
  ...
  private buildRoomFromState() {
    ...
    // Exit markers: small chevron at every connection tile so the player
    // can see where each room leads. Until we have a proper "door" sprite
    // generated by PixelLab, a tinted text glyph is the cheapest way to
    // make the exit affordance visible against the Wang tileset.
    for (const [dirRaw, exit] of Object.entries(map.exits)) {
      ...
      const arrow =
        ...
        : "◀";
      this.add
        .text(exit.x * RENDERED_TILE + RENDERED_TILE / 2, exit.y * RENDERED_TILE + RENDERED_TILE / 2, arrow, {
          color: "#FFD68A",
          fontSize: "14px",
          fontStyle: "bold",
        })
