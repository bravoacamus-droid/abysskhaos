 ...
 *
 * All icons are 32×32 PixelLab-generated PNGs hosted in R2 (content
 * hashed; URL is the source of truth for the binary). Renderable
 * directly via <img src> with `image-rendering: pixelated` for the
 * crisp pixel look matching the rest of the game.
 *
 * Generated in commit (phase-3d icon set wiring). If you need an
 * additional icon, generate via PixelLab MCP at size=32 with a
 * "small RPG UI icon" prompt + upload via .local-debug/upload-icons.ts
 * (entityType:'ui', entityId:`icon_<name>`).
 */
...
/** Category icons used in the inventory filter chips. WEAPON intentionally
 *  reuses the ATK sword silhouette — the generation timed out and a sword
 *  reads correctly for both "weapon category" and "attack stat" contexts. */
